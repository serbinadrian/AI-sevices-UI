...
import { ASC } from "./ASR_pb_service";
...
class AudioSceneRecognition extends React.Component {
  constructor(props) {
    super(props);
    ...
    this.state = {
      ...initialUserInput,
      users_guide: "https://github.com/iktina/audio-scene-recognition",
      code_repo: "https://github.com/iktina/audio-scene-recognition",
      reference: "https://github.com/iktina/audio-scene-recognition",
      response: undefined,
      data: undefined,
      currentDuration: 0
    };
    ...
    if (!this.state.isValid.validMeta || !this.state.isValid.validWAV) {
      const alertBox = (
        <p style={{
          ...
          Please provide a <b>.wav</b> file <b>up to 15 seconds</b> long and <b>up to 4 mb </b> in size!
        </p>);
  ...
  submitAction() {
    ...
    const methodDescriptor = ASC["audio2scene"];
    const request = new methodDescriptor.requestType();

    request.setData(data);
    request.setTimeFrame(timeFrame);

    console.log("aaa", request.getTimeFrame());
    ...
    const props = {
      ...
      onEnd: response => {
        ...
        this.setState({
          ...initialUserInput,
          response: { status: "success", data: message.getRes() },
        });
      },
    };
        ...
        <Grid container direction="column" justify="center" spacing={2}>
          ...
          <Grid item xs={12} sm={12} md={12} lg={12} className={classes.progressBarContainer}>
            <Grid item xs={12} sm={12} md={3} lg={3}>
              ...
              <span className={classes.title}>Step (sec)</span>
            </Grid>
            ...
            <Grid item xs={12} sm={12} md={9} lg={9} className={classes.sliderContainer}>
              ...
              <Slider
                ...
                onChange={(e, val) => this.changeSlider("timeFrame", val)}
              />
              <span className={classes.startEndNumber}>5</span>
            </Grid>
          </Grid>
  ...
  renderComplete() {
    const { response, timeFrame } = this.state;
    var frameResponse = [];
    var startFrameTime = 0;
    var endFrameTime = startFrameTime + timeFrame;

    var data = this.parseResponse(response.data);
    ...
    for (let index = 0; index < data.length; index++) {
      frameResponse.push(
        ...
        >
          <div style={{ width: "100px", flexShrink: "0" }}>
            <b>{"[ " + startFrameTime + " - " + endFrameTime + " ]"}:</b>
          </div>
          <div style={{ flex: "auto" }}>
            {data[index].join("; ")}
          </div>
      ...
      if (index === (data.length - 2)) {
        ...
        endFrameTime = startFrameTime + timeFrame;
      }
    }
    ...
    return (
      <div style={{ background: "#F8F8F8", padding: "24px" }}>
        <h4> Results</h4>
        <div style={{ padding: "10px 10px 0 10px", fontSize: "16px", color: "#9b9b9b" }}>
...
export default withStyles(useStyles)(AudioSceneRecognition);