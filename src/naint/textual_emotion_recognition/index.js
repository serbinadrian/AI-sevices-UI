...
import { TER } from "./textual_emotion_recognition_pb_service";
...
class TextualEmotionsRecognition extends React.Component {
  constructor(props) {
    const { state } = MODEL;
    super(props);
  ...
  parseResponse(response) {
    ...
    this.setState({
      response: JSON.parse(message.getResult()),
    });
  }
  ...
  submitAction() {
    ...
    const methodDescriptor = TER[service.METHOD];
    const request = new methodDescriptor.requestType();

    request.setText(this.constructRequest(textInputValue));
...
export default withStyles(useStyles)(TextualEmotionsRecognition);
