...
import { Corrector } from "./neural_grammar_correction_pb_service"; 
...
class NeuralGrammarCorrection extends React.Component {
  constructor(props) {
    const { state } = MODEL;
    super(props);
  ...
  parseResponse(response) {
    ...
    const [result] = JSON.parse(message.getResults());
  ...
  submitAction() {
    ...
    const methodDescriptor = Corrector[service.METHOD];
    const request = new methodDescriptor.requestType();
    // request.setData(textInputValue);
    request.setSentences(this.constructRequest(textInputValue));
...
export default withStyles(useStyles)(NeuralGrammarCorrection);